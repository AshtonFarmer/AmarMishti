/* SECRET DUCK UNIVERSE UNLOCK + RETURN FIX */
(function(){
  function showVideoPageFromDuckReturn(){
    const params = new URLSearchParams(window.location.search);
    if(params.get("duckReturn") !== "1") return;
  ...
  function css(){
    ...
    s.textContent=`
      ...
      .duck-secret-btn{
        ...
        font-weight:900;
        border:1px solid rgba(255,240,190,.95);
        background:linear-gradient(135deg,rgba(255,195,40,.58),rgba(255,120,200,.25));
        box-shadow:
          0 0 22px rgba(255,210,60,.72),
          0 0 44px rgba(255,0,160,.26),
          inset 0 0 14px rgba(255,255,255,.12);
        animation:duckSecretPulse 1.8s ease-in-out infinite;
      }

      @keyframes duckSecretPulse{
      ...
      .duck-secret-hint{
        ...
        font-size:.95rem;
        opacity:.86;
        text-shadow:0 0 12px rgba(255,0,160,.8);
      }
    `;
  ...
  function check(){
    if(document.querySelectorAll(".video-envelope.opened").length>=2){
      unlock();
    }
  }

  window.addEventListener("DOMContentLoaded",()=>{
    showVideoPageFromDuckReturn();
    css();
